---
seo_title: About Us - Teemane Cranes
seo_description: Established in 2012, Teemane Cranes prides itself on countless victories across South Africa's most demanding worksites. Learn about our mobile crane hire expertise.
---

# Page: About Us

Please fill out the content for each block below. If a block requires an image, place the image file in the `images/` folder and write the exact file name next to **Image:**.

---

## Block 1: Page Header

**Label:** Our Story
**Heading:** Redefining What's Possible In Mobile Crane Hire.
**Description:** Established in 2012, Teemane Cranes prides itself on countless victories across South Africa's most demanding worksites, from high-stakes projects to complex operations.

---

## Block 2: Stats Bar

**Stat 1 Value:** 2012
**Stat 1 Label:** Established
**Stat 2 Value:** 440T
**Stat 2 Label:** Max Lift Capacity
**Stat 3 Value:** 100+
**Stat 3 Label:** Years Combined Experience
**Stat 4 Value:** 24/7
**Stat 4 Label:** Emergency Response

---

## Block 3: Media & Text (Core Content)

**Heading:** Leading the projects that shape industries today.
**Image:** `Teemane-Boat.webp`
**Stat Card Value:** 440T
**Stat Card Label:** Maximum Fleet Capacity

**Content (Rich Text):**
Backed by a fleet of hydraulic cranes with capacities of up to 440 tons and a team driven to deliver unmatched results, we stand at the forefront of the sector. Every lift, transport, and rigging operation is executed with precision, safety, and efficiency at its core.

**Feature List:**
- **Proven Track Record:** Countless victories across South Africa's most demanding worksites since 2012.
- **Massive Capacity:** A modern fleet of hydraulic cranes with staggering capacities of up to 440 tons.
- **Unmatched Safety:** Stringent safety protocols ensuring zero-incident operations on high-stakes projects.
- **Precision Execution:** A highly skilled team driven to deliver unmatched results in complex operations.

---

## Block 4: Media & Text (Trusted Partner Section)

**Label:** Why Choose Us
**Heading:** Entrusted by South Africa's Most Established Companies.

**Content (Rich Text):**
Operating across some of the most advanced and complex sites, we are entrusted by South Africa's most established companies as their sole provider for full turnkey solutions.

Backed by more than 100 years of combined management experience, we bring the depth of knowledge, control and capability required to deliver certainty in high-risk environments and success on projects of the highest complexity.

**Button Label:** Work With Us
**Button URL:** /contact-us

**Feature Grid:**
- **Sole Provider:** Trusted as the exclusive full turnkey solutions partner by South Africa's most established companies.
- **100+ Years Experience:** Our management team brings over a century of combined industry experience to every project.
- **High-Risk Certainty:** The depth of knowledge, control, and capability required to deliver certainty in high-risk environments.
- **Complex Success:** A track record of success on projects of the highest complexity, delivered on time and to specification.
